...
func readPrivateKey() []byte {
	pkPEM, err := ioutil.ReadFile("/certs/input/key.pem")
	if err != nil {
		log.Fatal(err)
	}
...
func readCertificate() []byte {
	pkPEM, err := ioutil.ReadFile("/certs/input/cert.pem")
	if err != nil {
		log.Fatal(err)
	}
...
func main() {
	...
	writeKeyStore(ks1, "/certs/output/keystore_golang.jks", password)

	ks2 := readKeyStore("/certs/output/keystore_golang.jks", password)
